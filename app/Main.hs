...
main = do
  res <- cmdArgsRun mode
  case res of
    Synthesis jsonStr searchCat appMax logLv solNum noHigherOrder useRefine stopRef stopThreshold getNExamples getNTypes disableDema disableCoalescing coalescingStrategy noRelevancy noCopyTrans noBlacklist noFilter
      -> do
        ...
        let searchPrograms = if null jsonStr
              ...
              else executeSearch searchParams jsonStr
        case searchCat of
...
executeSearch :: SearchParams -> String -> IO ()
executeSearch searchParams inStr = catch
  (do
    ...
    _ <- synthesize searchParams goal exquery
    return ()
  )
  (\(e :: SomeException) -> do
