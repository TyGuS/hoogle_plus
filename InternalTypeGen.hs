...
instance {-# OVERLAPPABLE #-} (Show a) => Show (Inner a) where
  show (Inner x) = show x
instance {-# OVERLAPPING #-} (SF.ShowFunction a, Show a) => Show (Inner [a]) where
  show (Inner xs) = show $ map Inner xs
instance {-# OVERLAPPING #-} (SF.ShowFunction a, Show a) => Show (Inner (a, a)) where
  show (Inner p) = (show . over each Inner) p
instance {-# OVERLAPPING #-} (SF.ShowFunction (a -> b), Show a, Show b) => Show (Inner a -> Inner b) where
