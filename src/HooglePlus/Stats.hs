    ...
    modify $
        over
            ...
            (\s ->
                 case desc of
                     ConstructionTime ->
                         s {constructionTime = constructionTime s + (diff :: Double)}
...
printStats = do
    ...
    liftIO $
        putStrLn ("Petri net construction time: " ++ showFullPrecision (constructionTime stats))
    ...
    liftIO $
        putStrLn ("Hoogle plus type checking time: " ++ showFullPrecision (typeCheckerTime stats))
    ...
    liftIO $
        putStrLn
            ("Number of transitions: " ++ show (map snd (Map.toAscList (numOfTransitions stats))))
