...
distinguish' _ t1@(ScalarT (DatatypeT id1 tArgs1 _) _) t2@(ScalarT (DatatypeT id2 tArgs2 _) _)
    | id1 /= id2 = do
        ...
        return
            (Just
                 ( ScalarT (DatatypeT id1 freshArgs1 []) ()
                 , ScalarT (DatatypeT id2 freshArgs2 []) ()))
distinguish' tvs t1@(ScalarT (DatatypeT id1 tArgs1 _) _) t2@(ScalarT (DatatypeT id2 tArgs2 _) _)
    | id1 == id2 = do
        diffs <- firstDifference tArgs1 tArgs2
        case diffs of
            ([], []) -> return Nothing
            (ds1, ds2) ->
                return (Just (ScalarT (DatatypeT id1 ds1 []) (), ScalarT (DatatypeT id2 ds2 []) ()))
  where
...
distinguish' tvs t1@(ScalarT DatatypeT {} _) t2@(ScalarT (TypeVarT _ id) _)
    | id `elem` tvs = do
        diffs <- distinguish' tvs t2 t1
        case diffs of
            Nothing -> return Nothing
            Just d -> return (Just (swap d))
distinguish' tvs (ScalarT DatatypeT {} _) (ScalarT (TypeVarT _ id) _) =
    error "undecided actual type" -- return (Just (ScalarT (TypeVarT Map.empty id) ()))
distinguish' tvs t1@(ScalarT (TypeVarT _ id1) _) t2@(ScalarT (TypeVarT _ id2) _)
    | id1 `elem` tvs || id2 `elem` tvs = return (Just (t1, t2))
        ...
        Nothing ->
            case lookupSymbol ("(" ++ name ++ ")") 0 env of
                Nothing -> do
                    ...
                    writeLog 2 "findSymbol" $
                        text "cannot find symbol" <+>
                        text name <+> text "in the current environment"
                    return AnyT
...
updateCover tvs t cover =
    let (_, cover') = updateCover' tvs cover [] t rootNode
     in cover'
...
updateCover' bound cover intscts t paren
    | equalAbstract bound t paren = (intscts, cover)
updateCover' bound cover intscts t paren
    | isSubtypeOf bound t paren =
...
propagate env p@(Program (PSymbol sym) t) upstream = do
    ...
    unless
        (existAbstract bound cover upstream)
        (do let newCover = updateCover bound upstream cover
  ...
  where
    mostGeneral cArgs t = do
        ...
        lift $
            ...
            text "get generalized types" <+> pretty absArgs <+> text "from" <+> pretty cArgs
        res <- lift $ applySemantic bound t absArgs
        lift $
            writeLog 3 "propagate" $
            text "apply" <+>
            pretty absArgs <+> text "to" <+> pretty t <+> text "gets" <+> pretty res
        guard (isSubtypeOf bound res upstream)
        return $ map compactAbstractType absArgs
-- | case for lambda functions
propagate env (Program (PFun x body) (FunctionT _ tArg tRet)) (AFunctionT atArg atRet) =
...
bottomUpCheck env p@(Program (PSymbol sym) typ)
...
bottomUpCheck env p@(Program (PFun x body) (FunctionT _ tArg tRet)) = do
    ...
    ifM (gets $ view isChecked) (return $ Program (PFun x body') t) (return body')
    ...
    | isBound env id = do
        ...
        if id' `Map.member` (st ^. typeAssignment)
            then do
                ...
                writeLog 3 "solveTypeConstraint" $
                    text "Solving constraint" <+> pretty typ <+> text "==" <+> pretty tv
            ...
            then do
                ...
                solveTypeConstraint env typ tv'
                     ...
                     then do
                         let typ = fromJust $ Map.lookup id' $ st ^. typeAssignment
                         writeLog 3 "solveTypeConstraint" $
                             text "Solving constraint" <+> pretty tv <+> text "==" <+> pretty typ
                         solveTypeConstraint env tv typ
                     else unify env id tv'
solveTypeConstraint env tv@(ScalarT (TypeVarT _ id) _) t
    | isBound env id = modify $ set isChecked False
solveTypeConstraint env tv@(ScalarT (TypeVarT _ id) _) t = do
    st <- get
    writeLog 3 "solveTypeConstraint" $
        text "Solving constraint" <+> pretty tv <+> text "==" <+> pretty t
    if id `Map.member` (st ^. typeAssignment)
        then do
            let typ = fromJust $ Map.lookup id $ st ^. typeAssignment
            writeLog 3 "solveTypeConstraint" $
                text "Solving constraint" <+> pretty typ <+> text "==" <+> pretty t
...
solveTypeConstraint env (FunctionT _ tArg tRet) (FunctionT _ tArg' tRet') = do
    writeLog 3 "solveTypeConstraint" $
        text "Solving constraint" <+> pretty tArg <+> text "==" <+> pretty tArg'
    solveTypeConstraint env tArg tArg'
    writeLog 3 "solveTypeConstraint" $
        text "Solving constraint" <+> pretty tRet <+> text "==" <+> pretty tRet'
    solveTypeConstraint env tRet tRet'
solveTypeConstraint env (ScalarT (DatatypeT id tArgs _) _) (ScalarT (DatatypeT id' tArgs' _) _)
    | id /= id' = modify $ set isChecked False
solveTypeConstraint env (ScalarT (DatatypeT id tArgs _) _) (ScalarT (DatatypeT id' tArgs' _) _)
    | id == id' = solveTypeConstraint' env tArgs tArgs'
  where
    solveTypeConstraint' _ [] [] = return ()
    solveTypeConstraint' env (ty:tys) (ty':tys') = do
        writeLog 3 "solveTypeConstraint" $
            text "Solving constraint" <+> pretty ty <+> text "==" <+> pretty ty'
...
solveTypeConstraint env t1 t2 = do
    writeLog 3 "solveTypeConstraint" $
        text "unmatched types" <+> pretty t1 <+> text "and" <+> pretty t2
    modify $ set isChecked False
  ...
  where
    ...
    freshVars = do
        ...
        lift $
            ...
            text "generalize" <+> pretty t <+> text "into" <+> pretty absTy
        return absTy
    subsets [] = return []
    subsets (arg:args) = do
        args' <- subsets args
