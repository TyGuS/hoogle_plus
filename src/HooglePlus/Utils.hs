...
mkFunctionSigStr args = addConstraints $ Prelude.foldr accumConstraints ([], []) args
  where
    ...
    accumConstraints (ScalarT (DatatypeT id [ScalarT (TypeVarT _ tyvarName) _] _) _) (constraints, baseSigs)
        | tyclassPrefix `isPrefixOf` id =
            let classNameRegex = mkRegex $ tyclassPrefix ++ "([a-zA-Z]*)"
...
mkLambdaStr args body =
    let unTypeclassed = toHaskellSolution (show body)
     in unwords . words . show $ foldr addFuncArg (text unTypeclassed) args
  where
    addFuncArg arg rest
