...
checkStrictness tyclassCount body sig modules =
    handle (\(SomeException _) -> return False) (checkStrictness' tyclassCount body sig modules)
...
runGhcChecks params env goalType prog
    -- constructs program and its type signature as strings
 =
    let (modules, funcSig, body, argList) = extractSolution env goalType prog
     ...
     in do typeCheckResult <- liftIO $ runInterpreter $ checkType expr modules
           strictCheckResult <-
               if disableDemand
                   then return True
                   else liftIO $ checkStrictness tyclassCount body funcSig modules
           filterCheckResult <-
               if disableFilter
                   then return True
