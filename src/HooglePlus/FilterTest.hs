...
import Data.List
import qualified Data.Map as Map hiding (foldr, map)
import Data.Maybe
...
parseTypeString input = FunctionSignature constraints argsType returnType
  where
    ...
    buildSig constraints argList (TyForall _ _ (Just ctx) t) = buildSig constraints' argList t
      where
        constraints' = constraints ++ extractConstraints constraints ctx
    buildSig constraints argList (TyFun _ typeArg typeRet) = buildSig constraints argList' typeRet
      where
        argList' = argList ++ [extractType typeArg]
...
buildFunctionWrapper' wrapperName solution solutionType solutionParams timeInMicro =
    unwords [formatSolution solution solutionType, formatWrapper timeInMicro solutionParams]
  where
    ...
    formatWrapper timeInMicro params =
        printf
            "let %s %s = (CB.timeOutMicro' %d (CB.approxShow %d (sol_%s %s))) in"
            wrapperName
            params
            timeInMicro
...
buildNotCrashProp solution funcSig =
    (formatAlwaysFailProp argLine argDecl wrapper, formatNeverFailProp argLine argDecl wrapper)
  where
    ...
    formatProp propName body argLine argDecl wrappedSolution =
        unwords
            ([wrappedSolution] ++
             ...
             , "} in"
             , printf "quickCheckWithResult stdArgs { chatty = False } (%s)" propName
             ]) :: String
...
buildDupCheckProp (sol, otherSols) funcSig timeInMicro =
  ...
  where
    ...
    formatBindingList sols =
        let items = intercalate "," $ map formatBindingItem sols
         in printf "[%s]" items :: String
    formatProp wLhs wRhs argLine argDecl =
        unwords
            ([ printf "let dupProp %s = monadicIO $ do {" argDecl
             ...
             [ printf
                   "assert (Prelude.or $ Prelude.map (isEqualResult resultL) %s)"
                   (formatBindingList otherSols')
             , "} in"
             , printf "quickCheckWithResult stdArgs { chatty = False } dupProp"
             ]) :: String
...
validateSolution ::
    ...
    -> FunctionSignature
    -> Int
    -> IO (Either InterpreterError FunctionCrashKind)
...
compareSolution ::
    ...
    -> FunctionSignature
    -> Int
    -> IO (Either InterpreterError Result)
...
runChecks env goalType prog = and <$> mapM (\f -> f modules funcSig body) checks
  where
...
checkSolutionNotCrash modules sigStr body = liftIO executeCheck
  where
    handleNotSupported =
        (`catch` ((\ex -> print ex >> return True) :: NotSupportedException -> IO Bool))
    executeCheck =
        handleNotSupported $ do
...
toParamListDecl args = (plainArgLine, declArgLine)
  where
    n = length args
    ...
    computeAppDepth (ArgTypeFunc l r) =
        case r of
            ...
            _ -> 1
    computeAppDepth invalid =
        error $ printf "Invalid argument for computeAppDepth: %s" (show invalid)
    formatParam = printf "arg_%d" :: Int -> String
    toDecl :: (Int, ArgumentType) -> String
    toDecl (index, tipe@(ArgTypeFunc l r)) = printf "(%s arg_%d)" pattern index
      where
        pattern =
            case computeAppDepth tipe of
                1 -> "Fn"
                2 -> "Fn2"
                3 -> "Fn3"
