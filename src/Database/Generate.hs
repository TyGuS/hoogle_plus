...
import Synquid.Type (arity, isFunctionType, lastType, shape, toMonotype)
import Types.Generate
import Types.Type (RSchema, SType, TypeSkeleton(..))
...
readItem x
    | ParseOk y <- myParseDecl x = Just $ unGADT y
readItem x -- newtype
...
readItem x -- constructors
    | ParseOk (GDataDecl _ _ _ _ _ [GadtDecl s name _ ty] _) <-
         myParseDecl $ "data Data where " ++ x
    , let f (TyBang _ _ _ (TyParen _ x@TyApp {})) = x
          f (TyBang _ _ _ x) = x
          f x = x = Just $ TypeSig s [name] $ applyFun1 $ map f $ unapplyFun ty
...
readItem (stripPrefix "data (" -> Just xs) -- tuple data type
    | (com, ')':rest) <- span (== ',') xs
    , ParseOk (DataDecl a b c d e f) <-
         fmap unGADT $ myParseDecl $ "data " ++ replicate (length com + 2) 'A' ++ rest =
        Just $ DataDecl a b c ((op $ '(' : com ++ ")") d) e f
  where
    op s DHead {} = DHead () $ Ident () s
    op s x = x
readItem _ = Nothing
...
fixLine ('[':':':xs)
    | (a, ']':b) <- break (== ']') xs = "(:" ++ a ++ ")" ++ b
fixLine x
    | "class " `isPrefixOf` x = fst $ breakOn " where " x
fixLine x = x
