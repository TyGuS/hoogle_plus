...
supportedInnerType = ["Int", "Float", "Double", "Char", "String"]
...
instance Show ArgumentType where
    ...
    show (ArgTypeTuple types) = (printf "(%s)" . intercalate ", " . map show) types
    show (ArgTypeFunc src dst) = printf "((%s) -> (%s))" (show src) (show dst)

newtype NotSupportedException =
...
emptyFilterState = FilterState {inputs = [], solutions = []}
