...
absFunArgs id (AFunctionT tArg tRes)
    | id == "pair_match" = [tArg]
...
toAbstractType (ScalarT (DatatypeT id args _) _) =
    AScalar (ADatatypeT id (map (compactAbstractType . toAbstractType) args))
...
toAbstractFun (FunctionT _ tArg tRes) =
    AScalar $ ADatatypeT "Fun" [toAbstractFun tArg, toAbstractFun tRes]
toAbstractFun t = toAbstractType t

compactAbstractType :: AbstractSkeleton -> AbstractSkeleton
compactAbstractType (AFunctionT tArg tRes) =
    AScalar $ ADatatypeT "Fun" [compactAbstractType tArg, compactAbstractType tRes]
compactAbstractType (AScalar (ADatatypeT dt args)) =
    AScalar (ADatatypeT dt $ map compactAbstractType args)
...
checkUnification bound tass t1 t2
    | t1 == t2 = Just tass
    ...
    -- keep the most informative substitution, eagerly substitute into the final result
     =
        case checkUnification bound tass assigned t of
            Nothing -> Nothing
            Just u
                | isValidSubst u && isValidSubst (updatedTass u) -> Just (updatedTass u)
                | otherwise -> Nothing
  where
...
checkUnification bound tass (AScalar (ATypeVarT id)) t
    | id `elem` bound = Nothing
checkUnification bound tass t@(AScalar ADatatypeT {}) t'@(AScalar (ATypeVarT id)) =
    checkUnification bound tass t' t
checkUnification bound tass (AScalar (ATypeVarT id)) t
    | id `elem` abstractTypeVars t = Nothing
checkUnification bound tass (AScalar (ATypeVarT id)) t =
    let tass' = Map.map (abstractSubstitute (Map.singleton id t)) tass
        tass'' = Map.insert id t tass'
     in if isValidSubst tass''
            then Just tass''
            else Nothing
...
typeConstraints ::
       MonadIO m => [Id] -> AbstractSkeleton -> AbstractSkeleton -> PNSolver m [UnifConstraint]
typeConstraints bound (AFunctionT tArg tRet) (AFunctionT tArg' tRet') = do
    argCons <- typeConstraints bound tArg tArg'
    retCons <-
        if isAFunctionT tRet
            then typeConstraints bound tRet tRet'
            else return []
...
abstractSubstitute tass typ =
    if substed /= typ
        then abstractSubstitute tass substed
        else substed
  where
...
abstractSubstitute' id typ (AScalar (ATypeVarT id'))
    | id == id' = typ
abstractSubstitute' id typ t@(AScalar ATypeVarT {}) = t
abstractSubstitute' id typ (AScalar (ADatatypeT id' args)) =
    AScalar (ADatatypeT id' (map (abstractSubstitute' id typ) args))
...
existAbstract tvs cover t = existAbstract' rootNode
  where
    existAbstract' paren
        | equalAbstract tvs paren t = True
    existAbstract' paren
        | isSubtypeOf tvs t paren =
            any existAbstract' (Set.toList $ HashMap.lookupDefault Set.empty paren cover)
...
currentAbst tvs cover at = do
  ...
  where
    currentAbst' at paren
        | isSubtypeOf tvs at paren =
            let children = Set.toList $ HashMap.lookupDefault Set.empty paren cover
                inSubtree = any (isSubtypeOf tvs at) children
             in if inSubtree
                    then head $ filter isJust $ map (currentAbst' at) children
                    else Just paren
    currentAbst' at paren = Nothing

applySemantic ::
       MonadIO m => [Id] -> AbstractSkeleton -> [AbstractSkeleton] -> PNSolver m AbstractSkeleton
...
compareAbstract tvs t1 t2
    | isSubtypeOf tvs t1 t2 && isSubtypeOf tvs t2 t1 = EQ
