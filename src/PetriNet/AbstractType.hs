...
isAComp :: AbstractSkeleton -> Bool
isAComp ACompound {} = True
isAComp _ = False
...
encodeFunction :: Map AbstractSkeleton Int -> Id -> AbstractSkeleton -> FunctionCode
encodeFunction m id t | pairProj `isPrefixOf` id =
    let toMatch (FunctionCode name ho [p1,p2] ret) = FunctionCode id ho [p1] (p2:ret)
     in toMatch $ encodeFunction m "__f" t
encodeFunction m id t@(AFunctionT tArg tRet) = FunctionCode id hoParams params [res]
  where
    base = (0, [])
