...
unifySorts boundTvs = unifySorts' Map.empty
  where
    unifySorts' subst [] [] = Right subst
    unifySorts' subst (x:xs) (y:ys)
        | x == y = unifySorts' subst xs ys
    unifySorts' subst (SetS x:xs) (SetS y:ys) = unifySorts' subst (x : xs) (y : ys)
    ...
    unifySorts' subst (VarS x:xs) (y:ys)
        | not (Set.member x boundTvs) =
            case Map.lookup x subst of
                ...
                Nothing ->
                    if x `Set.member` typeVarsOfSort y
                        then Left (VarS x, y)
                        else unifySorts' (Map.insert x y subst) xs ys
    unifySorts' subst (x:xs) (VarS y:ys)
        | not (Set.member y boundTvs) = unifySorts' subst (VarS y : ys) (x : xs)
    unifySorts' subst (x:_) (y:_) = Left (x, y)
...
posNegUnknowns (Binary Implies e1 e2) =
    both2 Set.union (swap $ posNegUnknowns e1) (posNegUnknowns e2)
posNegUnknowns (Binary Iff e1 e2) =
    both2 Set.union (posNegUnknowns $ e1 |=>| e2) (posNegUnknowns $ e2 |=>| e1)
posNegUnknowns (Binary _ e1 e2) = both2 Set.union (posNegUnknowns e1) (posNegUnknowns e2)
posNegUnknowns (Ite e e1 e2) =
    both2 Set.union (posNegUnknowns $ e |=>| e1) (posNegUnknowns $ fnot e |=>| e2)
...
posNegPreds (Binary Iff e1 e2) =
    both2 Set.union (posNegPreds $ e1 |=>| e2) (posNegPreds $ e2 |=>| e1)
    ...
    case fml of
        ...
        SetComp v@(Var _ x) e ->
            if x `Map.member` subst
                then error $
                     unwords
                         [ "Scoped variable of the set comprehension clashes with substitution variable"
                         , x
                         ]
...
sortSubstituteFml subst fml =
    case fml of
        ...
        Ite c l r ->
            Ite (sortSubstituteFml subst c) (sortSubstituteFml subst l) (sortSubstituteFml subst r)
...
substitutePredicate pSubst fml =
    case fml of
        Pred b name args ->
            case Map.lookup name pSubst of
                Nothing -> Pred b name (map (substitutePredicate pSubst) args)
                Just value ->
                    substitute (Map.fromList $ zip deBrujns args) (substitutePredicate pSubst value)
        ...
        Ite e0 e1 e2 ->
            Ite
                (substitutePredicate pSubst e0)
                (substitutePredicate pSubst e1)
                (substitutePredicate pSubst e2)
    ...
    case fml of
        Unary Not e ->
            case e of
                ...
                Binary Iff e1 e2 ->
                    (negationNF e1 |&| negationNF (fnot e2)) |||
                    (negationNF (fnot e1) |&| negationNF e2)
                _ -> fml
        Binary Implies e1 e2 -> negationNF (fnot e1) ||| negationNF e2
        Binary Iff e1 e2 ->
            (negationNF e1 |&| negationNF e2) ||| (negationNF (fnot e1) |&| negationNF (fnot e2))
        Binary op e1 e2
            | op == And || op == Or -> Binary op (negationNF e1) (negationNF e2)
            | otherwise -> fml
        Ite cond e1 e2 ->
            (negationNF cond |&| negationNF e1) ||| (negationNF (fnot cond) |&| negationNF e2)
        _ -> fml
...
uDNF = dnf' . negationNF
  where
    dnf' e@(Binary Or e1 e2) =
        if (Set.null $ unknownsOf e1) && (Set.null $ unknownsOf e2)
            then return e
            else dnf' e1 ++ dnf' e2
...
fmlMeasure measures fml =
    case fml of
        ...
        Ite c t e ->
            (fmlMeasure measures c) `Set.union` (fmlMeasure measures t) `Set.union`
            (fmlMeasure measures e)
        Cons _ id args -> measures
        _ -> Set.empty

fmlVars :: Set Id -> Formula -> Set Id
fmlVars envVars fml =
    Set.map (\(Var s name) -> name) (varsOf fml) `Set.intersection` envVars `Set.difference`
    Set.singleton valueVarName
...
eliminateComp (Binary Subset l r)
    | hasComp l || hasComp r =
        let scopedVar = Var (elemSort $ sortOf l) "_comp"
         in All scopedVar (Binary Implies (setToPredicate scopedVar l) (setToPredicate scopedVar r))
eliminateComp (Binary Eq l r)
    | isSetS (sortOf l) && (hasComp l || hasComp r) =
        let scopedVar = Var (elemSort $ sortOf l) "_comp"
         in All scopedVar (Binary Iff (setToPredicate scopedVar l) (setToPredicate scopedVar r))
eliminateComp (Binary Neq l r)
    | isSetS (sortOf l) && (hasComp l || hasComp r) =
        error "Inequality on set comprehensions is not supported"
...
setToPredicate x (Binary Diff sl sr) =
    Binary And (setToPredicate x sl) (Unary Not (setToPredicate x sr))
...
lookupQuals qmap g (Unknown _ u) =
    case Map.lookup u qmap of
        Just qs -> view g qs
        Nothing -> error $ unwords ["lookupQuals: missing qualifiers for unknown", u]

lookupQualsSubst :: QMap -> Formula -> [Formula]
lookupQualsSubst qmap u@(Unknown s _) =
    concatMap go $ map (substitute s) (lookupQuals qmap qualifiers u)
  where
...
valuation sol (Unknown s u) =
    case Map.lookup u sol of
...
applySolution sol fml =
    case fml of
        Unknown s ident ->
            case Map.lookup ident sol of
...
instance Eq Candidate where
    (==) c1 c2 =
        Map.filter (not . Set.null) (solution c1) == Map.filter (not . Set.null) (solution c2) &&
        validConstraints c1 == validConstraints c2 && invalidConstraints c1 == invalidConstraints c2
