...
instance Pretty PredSig where
    pretty (PredSig p argSorts resSort) =
        hlAngles $
        text p <+>
        text "::" <+> hsep (map (\s -> pretty s <+> text "->") argSorts) <+> pretty resSort
...
fmlDocAt n fml =
    condHlParens
        ...
        (case fml of
             ...
             SetComp (Var s x) e ->
                 withSort (SetS s) (hlBrackets $ text x <> operator "|" <> pretty e)
             Var s name ->
                 withSort s $
                 if name == valueVarName
                     then special name
                     else text name
             Unknown s name ->
                 if Map.null s
                     then text name
                     else hMapDoc pretty pretty s <> text name
             Unary op e -> pretty op <> fmlDocAt n' e
             Binary op e1 e2 -> fmlDocAt n' e1 <+> pretty op <+> fmlDocAt n' e2
             Ite e0 e1 e2 ->
                 keyword "if" <+>
                 fmlDoc e0 <+> keyword "then" <+> fmlDoc e1 <+> keyword "else" <+> fmlDoc e2
...
prettyBase prettyType base =
    case base of
        IntT -> text "Int"
        BoolT -> text "Bool"
        DatatypeT "List" tArgs pArgs ->
            hlBrackets $ hsep (map (prettyType 0) tArgs) <+> hsep (map (hlAngles . pretty) pArgs)
        DatatypeT "Pair" (larg:rarg:[]) pArgs ->
            hlParens $
            prettyType 0 larg <+>
            operator "," <+> prettyType 0 rarg <+> hsep (map (hlAngles . pretty) pArgs)
        TypeVarT s name ->
            if Map.null s
                then text name
                else hMapDoc pretty pretty s <> text name
        DatatypeT name tArgs pArgs ->
            text name <+> hsep (map (prettyType 2) tArgs) <+> hsep (map (hlAngles . pretty) pArgs)
...
prettyTypeAt n t =
    condHlParens
        ...
        (case t of
             ...
             AnyT -> text "_"
             FunctionT x t1 t2 ->
                 hlParens (prettyTypeAt n' t1 <+> operator "->" <+> prettyTypeAt 0 t2)
             BotT -> text "⊥")
  where
    n' = typePower t
...
prettyCase cas =
    hang tab $
    text (constructor cas) <+>
    hsep (map text $ argNames cas) <+> operator "->" </> prettyProgram (expr cas)
    ...
    case p of
        ...
        PIf c t e ->
            ...
            (hang tab $
             keyword "if" <+>
             prettyProgram c $+$
             (hang tab (keyword "then" </> prettyProgram t)) $+$
             (hang tab (keyword "else" </> prettyProgram e)))
        PMatch l cases ->
            linebreak <>
            (hang tab $
             keyword "match" <+> prettyProgram l <+> keyword "with" $+$ vsep (map prettyCase cases))
        ...
        PLet x e e' ->
            ...
            (align $
             hang
                 tab
                 (keyword "let" <+>
                  withType (text x) (typeOf e) <+> operator "=" </> prettyProgram e </> keyword "in") $+$
             prettyProgram e')
        PHole ->
            if show (pretty typ) == dontCare
                then operator "??"
                else hlParens $ operator "?? ::" <+> pretty typ
        PErr -> keyword "error"
  where
...
instance Pretty MeasureCase where
    pretty (MeasureCase cons args def) =
        text cons <+> hsep (map text args) <+> text "->" <+> pretty def

instance Pretty MeasureDef where
    pretty (MeasureDef inSort outSort defs post) =
        nest
            2
            (pretty inSort <+>
             text "->" <+> pretty outSort <+> braces (pretty post) $+$ vsep (map pretty defs))

prettyBinding (name, typ) = text name <+> operator "::" <+> pretty typ

prettyBindings env =
    commaSep (map pretty (Map.keys $ removeDomain (env ^. constants) (allSymbols env)))
...
instance Pretty Candidate where
    pretty (Candidate sol valids invalids label) =
        text label <> text ":" <+>
        pretty sol <+> parens (pretty (Set.size valids) <+> pretty (Set.size invalids))

instance Show Candidate where
    show = show . plain . pretty

instance Pretty Goal where
    pretty (Goal name env spec impl depth _) =
        pretty env <+>
        operator "⊢" <+>
        text name <+>
        operator "::" <+>
        pretty spec $+$
        text name <+> operator "=" <+> pretty impl $+$ parens (text "depth:" <+> pretty depth)
...
prettyVarianceParam (predSig, contra) =
    pretty predSig <>
    (if contra
         then pretty Not
         else empty)
...
instance Pretty BareDeclaration where
    ...
    pretty (DataDecl name tParams pParams ctors) =
        ...
        hsep (map prettyVarianceParam pParams) <+> keyword "where" $+$ vsep (map pretty ctors)
    pretty (MeasureDecl name inSort outSort post cases isTermination) =
        hang tab $
        if isTermination
            then keyword "termination"
            else empty <+>
                 keyword "measure" <+>
                 text name <+>
                 operator "::" <+>
                 ...
                 if post == ftrue
                     then pretty outSort
                     else hlBraces (pretty outSort <+> operator "|" <+> pretty post) <+>
                          keyword "where" $+$ vsep (map pretty cases)
    ...
    pretty (InlineDecl name args body) =
        keyword "inline" <+> text name <+> hsep (map text args) <+> operator "=" <+> pretty body
...
prettyError (ErrorMessage ParseError pos descr) =
    align $
    hang tab $
    errorDoc
        (hcat $
         map
             (<> colon)
             [ text (sourceName pos)
             , pretty (sourceLine pos)
             , pretty (sourceColumn pos)
             , text " Parse Error"
             ]) $+$
    pretty descr
prettyError (ErrorMessage ResolutionError pos descr) =
    hang tab $
    errorDoc
        (hcat $
         map (<> colon) [text (sourceName pos), pretty (sourceLine pos), text " Resolution Error"]) $+$
    pretty descr
prettyError (ErrorMessage TypeError pos descr) =
    hang tab $
...
typeNodeCount (ScalarT (DatatypeT _ tArgs pArgs) fml) =
    fmlNodeCount' fml + sum (map typeNodeCount tArgs) + sum (map fmlNodeCount' pArgs)
...
programNodeCount (Program p _) =
    case p of
        ...
        PMatch e cases ->
            1 + programNodeCount e + sum (map (\(Case _ _ e) -> programNodeCount e) cases)
...
mkTableLaTeX widths docs = uncurry mkTable $ insertSeps widths docs
  where
    insertSeps widths docs = (intersperse 3 widths ++ [3], map ((++ [nl]) . intersperse tab) docs)
    tab = text " &"
    nl = text " \\\\"
...
instance Pretty SplitInfo where
    pretty (SplitInfo p r tr) =
        text "Split places:" <+>
        text (show p) $+$
        text "Removed transitions:" <+> text (show r) $+$ text "New transitions:" <+> text (show tr)
    ...
    pretty (FunctionCode name hop params rets) =
        ...
        hlBraces
            (text "function name:" <+>
             text name $+$
             text "HO parameters:" <+>
             hlBrackets (commaSep (map pretty hop)) $+$
             text "paramters:" <+>
