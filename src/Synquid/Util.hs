...
setCompare x y =
    case compare (Set.size x) (Set.size y) of
        EQ -> compare x y
        res -> res
...
boundedSubsets n s
    ...
    | otherwise =
        let (x, xs) = Set.deleteFindMin s
         in Set.map (Set.insert x) (boundedSubsets (n - 1) xs) `Set.union` boundedSubsets n xs -- x is in or x is out
...
toDisjointGroups m = toDisjointGroups' m []
  where
    ...
    toDisjointGroups' m acc
        | Map.null m = acc
        | otherwise =
            let ((key, vals), m') = Map.deleteFindMin m
             in let (keys', vals') = close (Set.singleton key) vals m'
                 in let m'' = removeDomain keys' m'
                     in toDisjointGroups' m'' ((keys', vals') : acc)
    close :: (Ord k, Ord v) => Set k -> Set v -> Map k (Set v) -> (Set k, Set v)
    close keys vals m =
        let (mDisj, mNonDisj) = Map.partition (disjoint vals) m
         in if Map.null mNonDisj
                then (keys, vals)
                else close
                         (keys `Set.union` Map.keysSet mNonDisj)
                         (vals `Set.union` (Set.unions $ Map.elems mNonDisj))
                         mDisj
...
groupTuples =
    map (\l -> (fst . head $ l, concatMap snd l)) . groupBy ((==) `on` fst) . sortBy (comparing fst)

showme :: (Show a) => String -> a -> a
showme label thing =
    let newlabel = Data.Text.Lazy.pack (label ++ ": \n")
     in pTraceShow (newlabel `Data.Text.Lazy.append` (pShow thing)) thing
