...
resolveSignatures foo@(FuncDecl name _)  = do
  trace ("signature: " ++ (show foo)) return foo
...
resolveSchema sch@(ForallT _ _) = do
    trace ("resolveSchema: " ++ (show sch)) return sch
    return sch
resolveSchema sch = do
  ...
  where
    ...
    resolveSchema' (Monotype t) = do
        trace ("resolveSchema' Monotype: " ++ (show t)) return undefined
...
substituteTypeSynonym name tArgs = do
  ...
  case Map.lookup name tss of
    Nothing -> error $ show $ text "Datatype or synonym" <+> text name <+> text "is undefined"
    Just (tVars, t) -> do
