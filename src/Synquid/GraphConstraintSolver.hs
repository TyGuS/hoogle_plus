...
nodeConstraint graph goal args edgeType v = do
    ...
    case edgeType of
        BoolVar -> do
            ...
            when (length inConsts > 0) (Z3.mkOr inConsts >>= Z3.mkImplies vConst >>= Z3.optimizeAssert)
...
dropConstraint :: MonadZ3 z3 => SuccinctGraph -> SuccinctType -> z3 ()
dropConstraint graph v = do
    let ins = allInOrOutEdges graph True v
    boolS <- Z3.mkBoolSort
    inSyms <- mapM Z3.mkIntSymbol ins
    vSym <- Z3.mkIntSymbol (fromJust $ elemIndex v $ allNodes graph)
...
getPathSolution graph edgeConsts nodeConsts edgeType = do
    ...
    case res of
        Sat -> do
            ...
            liftIO $ mapM print satEdges
            liftIO $ putStrLn $ "Selected nodes:"
            liftIO $ mapM print satNodes
...
addGraphConstraints graph goal args edgeType = do
    ...
    let edges = allEdges graph -- hashmap from succinct edges to their weights and indices
    -- liftIO $ print edges
    ...
    nodeSyms <- mapM Z3.mkIntSymbol [0..(length nodes - 1)]
    nodeConsts <- mapM (\sym -> Z3.mkConst sym boolS) nodeSyms
    ...
    constraints <- mapM (\comp -> do
        ...
        --         ) btmTier
        --     ) partitions
        ) $ filter (not . hasSuccinctFunction) $ filter isSuccinctComposite nodes
