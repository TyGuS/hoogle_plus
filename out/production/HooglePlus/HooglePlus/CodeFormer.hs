...
data FormerState = FormerState {
    typedTerms :: HashMap Int CodePieces,
...
applyFunction func = do
    args <- mapM (generateArgs [""]) (nub (permutations (funParams func)))
    let res = Set.fromList $ map (\a -> withParen $ fname ++ a) (concat args)
    -- update typedTerms
    st <- get
...
generateProgram :: [FunctionCode]      -- signatures used to generate program
                -> [Int]  -- argument types in the query
                -> [Id]                -- argument names
                -> [Int]  -- return types
