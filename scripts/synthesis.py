...
CMD_OPTS = ['--stop-refine', '--stop-threshold=10', '--disable-filter=False']
...
def run_benchmark(output_dir, results, name, query, examples, default_opts,
    ...
    if is_filtering:
        ...
        print('== Time ==', format_time(end - start), end=' ')
        if not solution or 'outError' in solution[0]:  # Synthesis failed
    ...
    for group in groups.values():
        for b in group.benchmarks:
            if b.name in results:
                ...
                print('Running', str(b))
                run_benchmark(output_dir, results, b.name, b.query, [],
